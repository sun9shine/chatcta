...
// Track a conversion (webhook-style, no auth, uses trackingId) — MUST be before router.use(auth)
...
router.get('/', async (req, res) => {
  try {
    ...
    const q = { userId: new mongoose.Types.ObjectId(req.user._id) };
    if (botId) q.botId = new mongoose.Types.ObjectId(botId);
    ...
    const totalValue = await Conversion.aggregate([
      { $match: { userId: new mongoose.Types.ObjectId(req.user._id), ...(botId ? { botId: new mongoose.Types.ObjectId(botId) } : {}), ...(platform ? { platform } : {}) } },
...
router.get('/daily', async (req, res) => {
  try {
    ...
    const data = await Conversion.aggregate([
      { $match: { userId: new mongoose.Types.ObjectId(req.user._id), createdAt: { $gte: since } } },
      { $group: {
